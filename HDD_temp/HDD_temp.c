...
#define reboot_delay 240	// The time delay between reboots
...
int main (int argc, char *argv[]) {
	...
	char *failure_prompt = "~0.";
	unsigned int rb_prmt_len = 0;
	unsigned int rb_fail_len = 0;
	unsigned int char_counter = 0;
	unsigned int char_fail_counter = 0;
	...
	while (counter<argc) {
		...
		found_string = -1;
		found_string = strcmp(argv[counter], "-f");
		if (found_string==0) {
			counter++;
			failure_prompt=argv[counter];
			printf("\e]P0Looking for the failure string \"%s\" before rebooting.\n", failure_prompt);
		}
	...
	// evaluate the compare string
	rb_fail_len = strlen(failure_prompt);

	printf("Looking for '%s' failure string before rebooting\n", failure_prompt);
	printf("String length of %d\n", rb_fail_len);
	...
	while (exit_loop==0) {
		...
		if (t >= t_trigger){
			printf("\n\n\e[1;33m\t >>>  Reboots = %0.0f. Failures =  %0.0f  <<< \n", reboot_counter, fail_counter);
			printf("Waiting for '%s' or '%s'\e[0m\n\n", reboot_prompt, failure_prompt);
			t_trigger = t + debug_time;
		}
...
//        	rdlen = read(fd, buf, sizeof(buf) - 1);		// waits for the buffer to get filled
		rdlen = read(fd, buf, 1);	// reads one character in at a time
			...
			if (*buf==reboot_prompt[char_counter]) {
				...
				char_counter = 0;
				if (*buf==failure_prompt[char_fail_counter]) {
					char_fail_counter++;
				}
				else {
					char_fail_counter=0;
			...
			if (char_counter==rb_prmt_len) {
				printf("\n\n\e[1;33mGood string found\nRebooting in 240 seconds...\e[0m\n");
		                relay_off(pin_num);
                		sleep (240);
		                relay_on(pin_num);
				char_counter=0;
			...
			if (char_fail_counter==rb_fail_len) {
				printf("\n\n\e[1;33mFailure string found\nRebooting in 240 seconds...\e[0m\n");
		                relay_off(pin_num);
                		sleep (240);
		                relay_on(pin_num);
				char_counter=0;
        	    ...
        	    printf("\e[1;33mError from read: %d: %s\e[0m\n", rdlen, strerror(errno));
	        }
	//usleep(75000);
	}
